...
#define SIGHASH_FORKID 0x40
            ...
            if (btchip_context_D.usingOverwinter) {
                ...
                dataBuffer[4] = 0; // support sprout transactions with empty joinsplit
                btchip_write_u32_le(dataBuffer + 4 + btchip_context_D.usingSprout, sighashType);
                PRINTF("Finalize hash with\n%.*H\n", sizeof(lockTime) + sizeof(sighashType) + btchip_context_D.usingSprout, dataBuffer);

                cx_hash(&btchip_context_D.transactionHashFull.sha256.header, CX_LAST,
                    dataBuffer, sizeof(lockTime) + sizeof(sighashType) + btchip_context_D.usingSprout, hash1, 32);
                PRINTF("Hash1\n%.*H\n", sizeof(hash1), hash1);
